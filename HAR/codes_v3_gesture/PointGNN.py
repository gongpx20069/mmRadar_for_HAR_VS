...
class PointGNN(nn.Module):
    ...
    def _conv1d_forward(self,x, state):
        ...
        for t in range(self.T):
            ...
            eij_output = self.MLP_f[t](eij_input)
            eij_output = eij_output.view(-1,x.size(1),x.size(1),eij_output.size(1))
            eij_output[~adj] = 0
            eij_output = torch.max(eij_output,dim=-2)[0]
...
class HAR_PointGNN(nn.Module):
    def __init__(self,r = 0.5,output_dim = 5, T = 3,conv1d_or_mlp = 'mlp',state_dim = 3, frame_num=60, extend = 128):
